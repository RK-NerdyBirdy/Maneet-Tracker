...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    optimizeDeps: {
      // react-simple-maps declares prop-types as a peerDependency — under
      // pnpm's strict layout it never lands in the root node_modules, so the
      // rolldown optimizer externalizes its `require("prop-types")` and the
      // browser crashes with "environment that doesn't expose the `require`
      // function". Installing it as a direct dep + force-bundling both here
      // keeps the require inside the optimized chunk.
      include: ['gsap', 'ogl', 'react-simple-maps', 'prop-types'],
      exclude: [],
    },
    server: {
      host: '0.0.0.0',
