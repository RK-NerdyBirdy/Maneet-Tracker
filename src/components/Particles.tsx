...
const vertex = /* glsl */ `
  ...
  void main() {
    ...
    float t = uTime;
    // Slow, continuous upward drift with wrap-around for a parallax starfield feel
    float range = uSpread * 2.0;
    float rise = mod(t * uDrift + random.x * range, range);
    mPos.y += rise - uSpread;
    // Gentle horizontal shimmer so stars twinkle rather than march in lockstep
    mPos.x += sin(t * 0.3 * random.z + 6.28 * random.w) * mix(0.05, 0.35, random.x);
    mPos.z += sin(t * 0.3 * random.w + 6.28 * random.y) * mix(0.05, 0.35, random.z);
...
const Particles = ({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      alphaParticles, particleBaseSize, sizeRandomness, cameraDistance, disableRotation, drift, pixelRatio]);
