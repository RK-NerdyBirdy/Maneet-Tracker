...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // The landscape dimensions we set earlier
    this.plane.scale.y = (this.viewport.height * (600 * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (1066 * this.scale)) / this.screen.width;
    ...
    // 🛠️ THE FIX: Re-calculate text scale to prevent distortion 🛠️
    if (this.title && this.title.mesh) {
      // 1. Get the original, un-stretched aspect ratio of the text texture
      const tex = this.title.mesh.program.uniforms.tMap.value;
      const aspect = tex.image.width / tex.image.height;
      
      // 2. Set the text height relative to the card (0.15 = 15% of card height)
      const localTextHeight = 0.15; 
      
      // 3. Counter-act the parent plane's horizontal stretch mathematically
      const localTextWidth = (this.plane.scale.y / this.plane.scale.x) * localTextHeight * aspect;
      
      // 4. Apply the corrected scale
      this.title.mesh.scale.set(localTextWidth, localTextHeight, 1);
      
      // 5. Keep it perfectly pinned below the card (-0.5 is the bottom edge)
      this.title.mesh.position.y = -0.5 - (localTextHeight * 0.5) - 0.05;
    }
  }
}
